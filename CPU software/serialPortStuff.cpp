 /*	This file contains functions that relate to serial ports in a general sense. 
	...
	There is an assumption here that anything connected to the serial port will use the common messaging format that
	was agreed upon where a packet is formatted:
...
{	// computes the checksum of the message starting with the byte in position 2.
	...
	uint8_t checksum = 0;
	for(size_t  I = 2; I < messageSize - 1; ++I)
	{
		checksum ^= message[I];
	}
