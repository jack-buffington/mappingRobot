				...
				if (actualChecksum == temp2)
					...
					for (int I = 0; I < 16; I++)
						...
						if(actualAngle < lastCabinAngle)
							...
							//##############################################################
							//##############################################################
							// It never sends out a scan without these lines...  Not sure why.
							// I originally put it in here to give an index to reference to see 
							// if this was a new scan or not.  
							rangeData temp;
							temp.distanceInMillimeters = ++scanCount;
							temp.angleInRadians = actualAngle * DEGREES_TO_RADIANS_MULTIPLIER;  
							partialScan.push_back(temp);
							}
