...
g++ mappingRobot.cpp serialPortStuff.cpp commonlyUsedFunctions.cpp -o mappingRobot
...
int main()
{
   ...
   if(setupSerialPort((char*)portString.c_str(), true))
   {
      ...
      sendTextToDisplay(0,0," Mapping Robot  ");
      sendTextToDisplay(1,0,"   CPU ready    ");
      ...
      sendTextToDisplay(1,0," Battery Voltage");
      sendTextToDisplay(2,0, trimmedVoltage);
      ...
      for(int I = 20; I > 0; --I)
      {
         ...
         sendTextToDisplay(1,7,numStr);
         sleep(1);
      }

      //sendTextToDisplay(1,0," Shutting down  ");
      sendTextToDisplay(2,0,"                ");
      ...
      sleep(1);
      //std::string responseString = exec("poweroff", false, false); 
...
void sendCPUready()
{   // This function tells the MCU that the CPU is fully booted.
   uint8_t messageBuffer[4];
   messageBuffer[0] = 171; // 0xAA
   messageBuffer[1] = 2;  // bytes in message
   messageBuffer[2] = 7;   // CPU ready
...
{   // This function tells the MCU that the CPU is fully booted.
   ...
   messageBuffer[0] = 171; // 0xAA
   messageBuffer[1] = 2;  // bytes in message
   messageBuffer[2] = 5;   // I'm shutting down
